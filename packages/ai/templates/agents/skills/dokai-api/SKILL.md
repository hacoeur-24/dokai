...
When asked to **set up API specs** (no spec exists yet under `DOKAI/openapi/`):

1. Read the repo's routes and controllers (see "How to author a spec from code" above, steps 1–6).
2. Write `DOKAI/openapi/<name>.yaml` (one file per independent API surface).
3. Summarize every endpoint documented and list any `TBD` items.

When asked to **update API specs** (a spec already exists):

1. Identify which routes/handlers changed.
2. Re-sync the spec following "How to update a spec when the API changes" above.
3. Summarize what was changed and any remaining `TBD`s.
